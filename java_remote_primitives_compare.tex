...
This paper presents a benchmarking suite that measures the performance of
...
timers that use both operating system timers and Java language
...
Java language and operating system timers, kernel packet filters, and
...
sockets and XML-RPC. In particular, the benchmarks support the
...
representation for the parameters and return values.  Yet, the frequent
...
performing all of the full GC events represent $55.23\%$ of the
...
these studies might be different from those produced by other nano,
micro, macro, combined, and application-specific benchmarks.  The
experiments only evaluate sockets and XML-RPC and thus they provide no
direct insights into the behavior of local-remote systems constructed
with other communication primitives.  Also, the client and server in
the current benchmarks exchange a limited variety of parameters and
return values.  However, our framework supports the integration of
other benchmarks and this paper reports on the results from using
benchmarks that are similar to those that were described by Allman
