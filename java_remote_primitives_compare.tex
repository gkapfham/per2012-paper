...
functionality.  This paper presents a suite of benchmarks that measure
the performance associated with using sockets and eXtensible Markup
Language remote procedure calls (XML-RPC) to exchange messages between
distinct address spaces that execute on the same computational node.
Furthermore, the paper reports on an empirical study comparing sockets
and XML-RPC with response time measurements from profilers that use
either operating system tools or Java language instrumentation.  The
benchmark suite also calculates network resource consumption by
employing packet filters within the GNU/\-Linux kernel.  Moreover, we
evaluate the response time results in light of memory subsystem
metrics that characterize the behavior of the Java virtual machine
(JVM).  Our findings indicate that sockets perform better when small
to very large objects are transmitted, whereas XML-RPC exhibits lower
response time than sockets with extremely large bulk data transfers.
Overall, the experiments reveal fundamental trade-offs in performance
and thus represent the first step towards determining whether or not
Java primitives can support efficient intra-node communication.
...
%%   of multi-core prcoessors there is a need to re-evaluate these
%%   findings in order to enrich the knowledge of intranode
%%   communication.
...
%% socket-based and XML-RPC intranode communication.  The use of this
...
% NOTE: Argh!  This means that this has to be a paramter to the algorithm
