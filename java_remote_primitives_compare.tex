...
time overheads.  In this case, the t-test rejects $H_0$ because socket
implementation of {\tt GRAB} exhibits no variability in the response
time metric.
...
$size(V)$ with \texttt{SV}. The experiments indicate that {\small
  ...
  $R_\Delta^\%(\mbox{{\tt
    ...
    primitive $P$ to transmit a vector with $k$ entries.}}  The very
low response time associated with socket-based communication when
...
over connection time and bandwidth.
...
\begin{table}[t]
  ...
  \fbox{All Response Times Calculated by the Java Timers}
...
\hline
 {\bf Comparison}     & {\bf CI for Sockets} & {\bf CI for XML-RPC} \\
\hline
...
Section~\ref{sec:future-work} will support the resolution of the
