...
benchmark execution are also explained in light of JVM behavior
...
instrumentation within the client source code to measure the fine
...
\begin{sloppypar}
Unless specified otherwise, this paper always views the change and
...
uses primitive $P'$ instead of $P$.  The framework also use the {\tt
  jvmstat} and {\tt hprof} tools to generate the JVM behavior profiles
...
appropriate entity in $\mathcal{J}$.
\end{sloppypar}
...
R_\mu (\mathcal{R}, B, P, N) = \frac{\displaystyle \sum_{R(B,P) \in \mathcal{R}} R(B,P)}{N}
\end{equation}
